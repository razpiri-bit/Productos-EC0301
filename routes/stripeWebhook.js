...
// Opcional correo (Postmark)
// const postmark = require('postmark');
// const pm = process.env.POSTMARK_SERVER_TOKEN ? new postmark.ServerClient(process.env.POSTMARK_SERVER_TOKEN) : null;
...
// Función opcional para enviar el correo con el código
...
router.post('/stripe/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    if (event.type === 'checkout.session.completed') {
      const session = event.data.object; // Session con customer_email y metadata [web:120]
      ...
      if (!email) {
        ...
        return res.status(200).json({ received: true }); // evitar reintentos
      }

      const code = nanoid(10);
      const expiresAt = new Date(Date.now() + 1000 * 60 * 60 * 24 * 90); // 90 días

      // 3) Insert idempotente (requiere UNIQUE en stripe_event_id)
      ...
      console.log('Access code upserted:', {
      ...
      // 4) Enviar correo (si configuraste Postmark)
